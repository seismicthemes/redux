 ...
 */
function toolbox_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'toolbox' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'toolbox_wp_title', 10, 2 );
...
function theme_scripts() {
	...
	wp_enqueue_style('toolbox', get_bloginfo('stylesheet_url'), false, $theme_version);
	wp_enqueue_style('columns', get_stylesheet_directory_uri().'/css/3c-l.css', false, $theme_version);

	wp_enqueue_script( 'jquery' );
	wp_enqueue_script( 'menu-toggle', get_template_directory_uri() . '/js/menu.js');
